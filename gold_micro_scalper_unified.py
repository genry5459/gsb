...
class CapitalConfig:
    ...
    login: str = os.getenv("CAPITAL_LOGIN", "")
    password: str = os.getenv("CAPITAL_PASSWORD", "")
    demo: bool = os.getenv("CAPITAL_DEMO", "true").lower() == "true"
...
class CapitalClient:
    ...
    def _ensure_session(self) -> None:
        if time.time() < self.session_expires - 60 and self.cst and self.security_token:
            return
        self._create_session()

    def _create_session(self) -> None:
        log.info("Creating Capital.com session...")
        ...
        if resp.status_code != 200:
            raise ConnectionError(f"Session failed: {resp.status_code} {resp.text[:500]}")
        self.cst = resp.headers.get("CST", "")
        self.security_token = resp.headers.get("X-SECURITY-TOKEN", "")
        self.session_expires = time.time() + 600
        accounts = resp.json().get("accounts", [])
        for acc in accounts:
            if acc.get("accountType") == "TRADING":
                self.account_id = acc.get("accountId", "")
                break
        if not self.account_id and accounts:
            self.account_id = accounts[0].get("accountId", "")
        log.info("Session OK (%s)", "DEMO" if self.demo else "LIVE")
    ...
    def search_market(self, query: str) -> List[Dict[str, Any]]:
        data = self._request("GET", "/markets", params={"searchTerm": query, "pageSize": 10})
        return data.get("markets", [])

    def get_epic(self, symbol: str) -> str:
        if symbol in self._epic_cache:
            return self._epic_cache[symbol]
        markets = self.search_market(symbol)
        if not markets:
            raise ValueError(f"Market not found: {symbol}")
        for m in markets:
            name = str(m.get("instrumentName", "")).upper()
            epic = str(m.get("epic", ""))
            if symbol.upper() in name or symbol.upper() == epic.upper():
    ...
    def get_candles(self, epic: str, timeframe: str, count: int = 500) -> List[Dict[str, Any]]:
        tf = TIMEFRAME_MAP.get(timeframe, timeframe)
        data = self._request(
            "GET",
            f"/prices/{epic}",
            params={"resolution": tf, "max": count, "pageSize": min(count, 1000)},
        )
        rows = []
        for c in data.get("prices", []):
            try:
                ts = c.get("snapshotTime")
                if not ts:
                    continue
                ts = ts.replace("BST", "+01:00").replace("GMT", "+00:00")
                stamp = datetime.fromisoformat(ts)
                rows.append(
                    {
                        "time": stamp,
                        "open": safe_float(c["openPrice"]["bid"]),
                        "high": safe_float(c["highPrice"]["bid"]),
                        "low": safe_float(c["lowPrice"]["bid"]),
                        "close": safe_float(c["closePrice"]["bid"]),
                        "volume": safe_float(c.get("lastTradedVolume", 0.0)),
                    }
                )
            except Exception:
                continue
    ...
    def get_market_price(self, epic: str) -> Dict[str, float]:
        ...
        return {
            ...
            "epic": epic,
            "instrument_name": data.get("instrument", {}).get("name", ""),
        }
        ...
        for acc in data.get("accounts", []):
            if acc.get("accountId") == self.account_id:
                return {
                    ...
                    "available": safe_float(acc.get("available")),
                    "pnl": safe_float(acc.get("pnl")),
    ...
    def open_position(
        ...
        if stop_level is not None:
            payload["stopLevel"] = round(float(stop_level), 2)
        if limit_level is not None:
            payload["limitLevel"] = round(float(limit_level), 2)
        try:
            data = self._request("POST", "/positions/otc", json=payload)
            return OrderResult(success=True, deal_id=data.get("dealReference", ""), raw=data)
    ...
    def modify_position(
        ...
        if limit_level is not None:
            payload["limitLevel"] = round(float(limit_level), 2)
        try:
            data = self._request("PUT", "/positions/otc", json=payload)
    ...
    def close_position(self, deal_id: str) -> OrderResult:
        try:
            data = self._request("DELETE", "/positions/otc", json={"dealId": deal_id})
            return OrderResult(success=True, deal_id=data.get("dealReference", deal_id), raw=data)
        except Exception as exc:
            return OrderResult(success=False, error=str(exc))
...
class MicroScalpBot:
    ...
    def start(self) -> None:
        ...
        self.epic = self.client.get_epic(TRADING.symbol)
        log.info("Trading %s (EPIC: %s)", TRADING.symbol, self.epic)
    ...
    def fetch_candles(self, count: int = 500) -> pd.DataFrame:
        raw = self.client.get_candles(self.epic, TRADING.timeframe, count)
        if not raw:
    ...
    def get_current_price(self) -> Dict[str, float]:
        return self.client.get_market_price(self.epic)
...
def _check_env_or_die() -> None:
    """Validate that required Capital.com credentials are present."""
    missing = []
    ...
    log.info("ENV OK: CAPITAL_API_KEY is set (length=%d)", len(api_key))
    log.info("ENV OK: CAPITAL_LOGIN is set (%s***)", login[:3])
    log.info("ENV OK: CAPITAL_PASSWORD is set (length=%d)", len(password))
