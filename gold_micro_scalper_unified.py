...
class OandaClient:
    ...
    def get_candles(self, instrument: str, granularity: str, count: int = 500) -> List[Dict[str, Any]]:
        log.info("OANDA candles: instrument=%s granularity=%s count=%d", instrument, granularity, count)
        params = {
            "count": count,
...
class MicroScalpBot:
    ...
    def start(self) -> None:
        ...
        log.info("=" * 72)
        if not OANDA.api_token or not OANDA.account_id:
            log.warning("OANDA credentials are incomplete. Live mode will fail until .env is filled.")
    ...
    def fetch_candles(self, count: int = 500) -> pd.DataFrame:
        granularity = TIMEFRAME_MAP.get(TRADING.timeframe, TRADING.timeframe)
        raw = self.client.get_candles(self.instrument, granularity, count)
        if not raw:
    ...
    def _refresh_strategy(self, df: pd.DataFrame) -> None:
        if time.time() - self.last_strategy_refresh < 300:
            log.info("Strategy refresh skipped (cooldown %.0fs)", time.time() - self.last_strategy_refresh)
            return
        self.last_strategy_refresh = time.time()
        log.info("Refreshing strategy ranking...")
        ...
        if len(ranking) == 0:
            log.warning("Strategy ranking empty")
            return
        ...
        if best_name in self.strategy_names:
            ...
            log.info("Selected best strategy: %s (score=%.3f)", best_name, self.active_strategy_score)
        else:
            log.warning("Best strategy %s not found in list", best_name)

    def get_signal(self, df: pd.DataFrame) -> int:
        strat = ALL_STRATEGIES[self.active_strategy_idx]
        sig = strat.generate_signals(df)
        val = int(sig.iloc[-1]) if len(sig) else 0
        log.info("Signal from %s: %d (last %d bars checked)", strat.name, val, len(sig))
        # Логируем ключевые индикаторы последнего бара
        if len(df) > 0:
            last = df.iloc[-1]
            log.info("Last bar: close=%.2f rsi_14=%.1f adx=%.1f vol_ratio=%.2f ema_spread=%.4f",
                     last.get("Close", 0), last.get("rsi_14", 0), last.get("adx", 0),
                     last.get("vol_ratio", 0), last.get("ema_spread", 0))
        return val
    ...
    def run_once(self) -> None:
        ...
        if self.active_strategy_score < TRADING.min_live_strategy_score:
            log.info(
                "Strategy score %.3f below threshold %.3f, but continuing anyway",
                self.active_strategy_score,
                TRADING.min_live_strategy_score,
            )
            # NOTE: score check disabled for M1 to allow trading with limited history
